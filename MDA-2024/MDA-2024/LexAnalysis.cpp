...
namespace Lexer
{
	...
	int getLiteralIndex(IT::IdTable ittable, char* value, IT::IDDATATYPE type, Log::LOG log, int line) // получаем индекс повторного литерала(по значению)
	{
					...
					case LEX_LITERAL:
					{
						...
						else // повторный идентификатор (уже есть)
						{
							...
							if (lexema == LEX_LITERAL)
								idxTI = getLiteralIndex(tables.idtable, curword, getType(id, in.words[i - 1].word), log, curline); // или литерала
						}
					}
					break;
					}
